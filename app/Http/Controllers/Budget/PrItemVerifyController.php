...
class PrItemVerifyController extends Controller
{
    public function index(Request $request)
    {
        abort_if(Gate::denies('purchase_request_item_verify_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        $id = $request->id;
        $data = PurchaseRequest::find($id);
        $total_cost =  DB::table('purchase_request_items')->where('purchase_request_id', $id)->sum('total_cost');  

        return view('budget.purchase_request_item_verify.index', compact('id', 'data', 'total_cost'));
    }
