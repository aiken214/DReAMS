...
class StockCardController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('supply.stock_card.index', compact('id', 'data', 'item', 'description', 'office'));
    }

    public function getStockCards(Request $request)
    {                     
        // Fetch data
        $id = $request->stock_card_id; 
       
        $risItems = RisItem::with(['ris', 'stocks'])->where('stock_card_id', $id)->get();
        ...
        return datatables($risItems)
            ...
            ->editColumn('days_to_consume', function($data){
                return null;
            })
            ->make(true);   
