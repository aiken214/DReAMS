...
<script>
    ...
    $(function () {
        ...
        let table = $('#ppmpTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.ppmp_approve.index') }}',
            buttons: dtButtons,
            columns: [
                ...
                { data: 'remarks', name: 'remarks' },
                {
                    data: 'id',
                    render: function (data, type, row) {
                        return `@can('ppmp_approve_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('sds.ppmp_approve.show', '') }}/${data}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            @can('ppmp_item_approve_access')
                            <a class="btn btn-xs btn-success" href="{{ route('sds.ppmp_item_approve.index2', '') }}/${data}">
                                {{ trans('global.item') }}
                            </a>
                            @endcan
        ...
        $("#approvePpmpForm").submit(function (e) {
            ...
            $.ajax({
                url: '{{ route('sds.ppmp_approve.approve', '') }}/' + $('#approvePpmpForm #id').val(),
                method: "POST",
                data: fd,
                cache: false,
                contentType: false,
