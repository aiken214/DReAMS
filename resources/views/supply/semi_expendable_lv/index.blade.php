...
<div class="card">
    <div class="card-header">
        {{ trans('cruds.common.fields.low_value') }} {{ trans('cruds.semi_expendable.title_singular') }} {{ trans('cruds.common.fields.property') }} {{ trans('global.list') }}
    </div>
...
<script>
    $(function () {
        ...
        let table = $('#rrspTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.semi_expendable_lv.index') }}', // Your API route
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('semi_expendable_card_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.semi_expendable_lv.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('semi_expendable_card_access')
                                <a class="btn btn-xs btn-success" href="{{ route('supply.semi_expendable_card_lv.index2', '') }}/${data}">
                                    {{ trans('global.item') }}
                                </a>
                            @endcan`;
                    },
