...
<script>
    $(function () {
        ...
        let table = $('#rrspTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.stock.index') }}', // Your API route
            ...
            columns: [
                ...
                { data: 'remarks', name: 'remarks' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('stock_card_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.stock.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('stock_card_access')
                                <a class="btn btn-xs btn-success" href="{{ route('supply.stock_card.index2', '') }}/${data}">
                                    {{ trans('global.item') }}
                                </a>
                            @endcan`;
                    },
