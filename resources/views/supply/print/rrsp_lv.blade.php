        ...
        <div class="row">
          ...
          <div class="col-lg-12" style="margin-top: 20px;">
              ...
              <div id="test">
                ...
                <p style="text-align:center; font-size: 20px; bottom-margin: 30px;"><b>RECEIPT OF RETURNED SEMI-EXPENDABLE PROPERTY</b></p>

                <table style="width: 100%; margin: 0px 15px 0px 15px" cellspacing="0" cellspading="0">                    
                    <tr>
                        <td style="width: 65%">                    
                            <p style="text-align: left; vertical-align: top; font-size: 12px; margin-left: 5px">Entity Name: &nbsp;<b>{{ $rrspData->ics_lv->entity_name }}</b></p> 
                        </td>
                        <td style="width: 35%"> 
                            <p style="vertical-align: top; font-size: 12px; margin-left: 20px">Date: &nbsp;<b>{{ $rrspData->date }}</b>
                            <br>RRSP No.: &nbsp;<b>{{ $rrspData->rrsp_lv_no }}</b></p>              
                        </td>
                    </tr>
                </table>
                <table style="width: 100%; margin: 10px 15px 10px 15px," cellspacing="0" cellspading="0">
                      ...
                      @foreach ($items as $data)
                        <tr>
                            <td style="padding-left: 5px; font-size: 14px; border: 1px solid black">{{ $data->description }}</td>
                            <td style="text-align:center; font-size: 14px; border: 1px solid black">{{ $data->quantity }}</td>                             
                            <td style="text-align:center;  font-size: 14px; border: 1px solid black">{{ $rrspData->ics_lv->ics_lv_no }}</td> 
                            <td style="text-align:center;  font-size: 14px; border: 1px solid black">{{ $rrspData->ics_lv->recipient }} <br> {{ $rrspData->ics_lv->designation }} </td> 
                            <td style="padding-left: 5px; font-size: 14px; border: 1px solid black">{{ $data->remarks }}</td>                                                               
                        </tr>                        
                      @endforeach
                        ...
                        <td width="50%" style="font-size: 12px; border: 1px solid black">                    
                            ...
                            <div class="row justify-content-center" style="margin-top: 90px;">
                              <p style="text-align: center; font-size: 12px; margin: 1px; padding: 1px">
                                <b><u>{{ strtoupper($rrspData->ics_lv->recipient) }}</u></b>
                                <br>End User                                                      
                                <br><br><b><u>{{ $rrspData->date}}</u></b>
                                <br>Date
                              </p>                                   
                            </div> 
                        </td> 
