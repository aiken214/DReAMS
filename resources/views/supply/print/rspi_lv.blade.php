              ...
              <div id="test">
                ...
                <table style="width: 100%; margin: 10px 15px 10px 15px," cellspacing="0" cellspading="0">
                      ...
                      <tr>
                        ...
                        @php
                            $rowspan_counts = []; // Initialize an array to store counts of rows per ics_lv_no
                        @endphp

                        {{-- Count occurrences of each ics_lv_no --}}
                        @foreach ($data as $datum)
                            @php
                                $ics_lv_no = $datum->ics_lv->ics_lv_no ?? 'N/A'; // Use 'N/A' if ics_lv_no is null
                                if (!isset($rowspan_counts[$ics_lv_no])) {
                                    $rowspan_counts[$ics_lv_no] = 0;
                                }
                                $rowspan_counts[$ics_lv_no]++;
                            @endphp
                        @endforeach
                        @php
                            $printed_ics_lv_no = []; // Array to track printed ics_lv_no
                        @endphp
                        @foreach ($data as $datum)
                            @php
                                $ics_lv_no = $datum->ics_lv->ics_lv_no ?? 'N/A';
                            @endphp
                            <tr>
                                {{-- Display ics_lv_no with rowspan on its first occurrence --}}
                                @if (!isset($printed_ics_lv_no[$ics_lv_no]))
                                    <td rowspan="{{ $rowspan_counts[$ics_lv_no] }}" style="text-align:center; vertical-align:middle; font-size: 12px; border: 1px solid black;">
                                        {{ $ics_lv_no }}
                                    </td>
                                    @php
                                        $printed_ics_lv_no[$ics_lv_no] = true; // Mark this ics_lv_no as printed
                                    @endphp
                                @endif
                                {{-- Other table cells --}}
                                <td style="text-align:center; font-size: 12px; border: 1px solid black;">{{ '01' }}</td>
                                <td style="text-align:center; font-size: 12px; border: 1px solid black;">{{ $datum->inventory_item_no }}</td>
                                <td style="font-size: 12px; border: 1px solid black;">{{ $datum->description }}</td>
                                <td style="text-align:center; font-size: 12px; border: 1px solid black;">{{ $datum->quantity }}</td>
                                <td style="text-align:center; font-size: 12px; border: 1px solid black;">{{ $datum->unit }}</td>
                                <td style="text-align:right; font-size: 12px; border: 1px solid black;">{{ $datum->unit_cost }}</td>
                                <td style="text-align:right; font-size: 12px; border: 1px solid black;">{{ $datum->total_cost }}</td>
                            </tr>
                        @endforeach
