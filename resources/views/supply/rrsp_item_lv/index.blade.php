...
@can('rrsp_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('supply.rrsp_item_lv.create2', $id) }}">
                {{ trans('global.add') }} {{ trans('cruds.rrsp.title_short') }}
            </a>
            <a class="btn btn-secondary" href="{{ route('supply.rrsp_lv_print', $id) }}">
                {{ trans('global.view') }} {{ trans('cruds.rrsp.title_short') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable" id="rrsp_itemTable">
                <thead>
                    <tr>
                        <th width="10">
            ...
            <input type="hidden" name="hidden_rrsp_lv_id" id="hidden_rrsp_lv_id" value="{{ $id }}"/>   
        </div>
    </div>
</div>
...
<script>
    var rrsp_lv_id = $('#hidden_rrsp_lv_id').val();   
...
<script>
    $(function () {
        ...
        let table = $('#rrsp_itemTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.rrsp_item_lv.index') }}?rrsp_lv_id='+rrsp_lv_id, // Your API route
            ...
            columns: [
                ...
                { data: 'type', name: 'type' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('rrsp_item_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.rrsp_item_lv.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('rrsp_item_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('supply/rrsp_item_lv') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('rrsp_item_delete')
                                <form action="{{ route('supply.rrsp_item_lv.destroy', '') }}/${data}" method="POST" 
