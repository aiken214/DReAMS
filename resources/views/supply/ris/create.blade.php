...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("supply.ris.store") }}" enctype="multipart/form-data">
            @csrf
            <input type="text" name="purchase_order_id" id="purchase_order_id" hidden>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('iar_id') ? 'is-invalid' : '' }}" name="iar_id" id="iar_id" required>
                    ...
                    @foreach($iars as $iar)
                        <option value="{{ $iar->id }}" 
                            data-purchase_order_id="{{ $iar->purchase_order_id }}" 
                            {{ old('iar_id') == $iar->id ? 'selected' : '' }}>
                            {{ $iar->iar_no }} - {{ $iar->purchase_order->purchase_request->office}}
                        </option>
                    @endforeach   
                </select>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const purchaseOrderIdField = document.getElementById('purchase_order_id');
        ...
        iarIdDropdown.addEventListener('change', function () {
            ...
            const purchase_order_id = selectedOption.getAttribute('data-purchase_order_id');

            // Set the amount field's value
            purchaseOrderIdField.value = purchase_order_id || ''; // Default to empty if no amount is provided
