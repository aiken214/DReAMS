    ...
    <div class="card-body">
        <form method="POST" action="{{ route("supply.ris.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="asset_id">{{ trans('cruds.ris.fields.asset_no') }}</label>
                <select class="form-control {{ $errors->has('asset_id') ? 'is-invalid' : '' }}" name="asset_id" id="asset_id" required>
                    <option value="" disabled {{ old('asset_id', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}
                    </option>
                    @foreach($assets as $asset)
                        <option value="{{ $asset->id }}" 
                            data-purchase_order_id="{{ $asset->purchase_order_id }}" 
                            {{ old('asset_id') == $asset->id ? 'selected' : '' }}>
                            {{ $asset->asset_no }} - {{ $asset->purpose}}
                        </option>
                    @endforeach   
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.ris.fields.asset_no_helper') }}</span>
            </div>  
            <div class="form-group">
