...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("supply.ris.store") }}" enctype="multipart/form-data">
            @csrf
            <input type="text" name="purchase_request_id" id="purchase_request_id" hidden>
            <input type="text" name="employee_id" id="employee_id" hidden>
            <div class="form-group">
                <label class="required" for="iar_id">{{ trans('cruds.ris.fields.iar_no') }}</label>
                <select class="form-control {{ $errors->has('iar_id') ? 'is-invalid' : '' }}" name="iar_id" id="iar_id" required>
                    <option value="" disabled {{ old('iar_id', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}
                    </option>
                    @foreach($iars as $iar)
                        <option value="{{ $iar->id }}" 
                            data-purchase_request_id="{{ $iar->purchase_request_id }}" 
                            {{ old('iar_id') == $iar->id ? 'selected' : '' }}>
                            {{ $iar->iar_no }} - {{ $iar->purchase_request->office}}
                        </option>
                    @endforeach   
                </select>
                @if($errors->has('iar_id'))
                    <span class="text-danger">{{ $errors->first('iar_id') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.ris.fields.iar_no_helper') }}</span>
            </div>  
            <div class="form-group">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const iarIdDropdown = document.getElementById('iar_id');
        const purchaseOrderIdField = document.getElementById('purchase_request_id');
        const recipientDropdown = document.getElementById('recipient');
        const employeeIdField = document.getElementById('employee_id');

        iarIdDropdown.addEventListener('change', function () {
            const selectedOption = iarIdDropdown.options[iarIdDropdown.selectedIndex];
            const purchase_request_id = selectedOption.getAttribute('data-purchase_request_id');

            // Set the amount field's value
            purchaseOrderIdField.value = purchase_request_id || ''; // Default to empty if no amount is provided
        });
