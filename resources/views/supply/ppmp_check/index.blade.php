...
@can('ppmp_check_access')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('supply.ppmp_check.checked') }}">
                {{ trans('global.checked') }} {{ trans('cruds.ppmp.title_singular') }} {{ trans('global.list') }}
            </a>            
        </div>
    </div>
@endcan
...
<script>
    ...
    $(function () {
        ...
        let table = $('#ppmpTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.ppmp_check.index') }}',
            buttons: dtButtons,
            columns: [
                { data: null, "defaultContent": "", orderable: false, className: 'select-checkbox' },
                { data: 'id', name: 'id' },
                { data: 'calendar_year', name: 'calendar_year' },
                { data: 'title', name: 'title', render: function (data) { return '<span style="white-space:normal">' + data + '</span>'; }},
                { data: 'type', name: 'type' },
                { data: 'category', name: 'category' },
                { data: 'prepared_by', name: 'prepared_by', render: function (data) { return '<span style="white-space:normal">' + data + '</span>'; }},
                { data: 'station', name: 'station', render: function (data) { return '<span style="white-space:normal">' + data + '</span>'; }},
                ...
                { data: 'remarks', name: 'remarks' },
                {
                    data: 'id',
                    render: function (data, type, row) {
                        return `@can('ppmp_check_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('supply.ppmp_check.show', '') }}/${data}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            @can('ppmp_item_check_access')
                            <a class="btn btn-xs btn-success" href="{{ route('supply.ppmp_item_check.index2', '') }}/${data}">
                                {{ trans('global.item') }}
                            </a>
                            @endcan
                            @can('ppmp_check_access')
                                ${(row.finalized !== null && row.checked === null && row.remarks !== 'Request for PPMP reversal - End User.') ? `                                    
                            ...
                            @endcan
                            @can('ppmp_check_access')
                            ${row.remarks === 'Request for PPMP reversal. - End User' ? `
                                <a class="btn btn-xs btn-secondary text-white revertIcon" href="{{ route('supply.ppmp_check.revert', '') }}/${data}">
                                    {{ trans('global.revert') }}
                                </a>
                            ` : ''}
                            @endcan`;
        ...
        $("#approvePpmpForm").submit(function (e) {
            ...
            $.ajax({
                url: '{{ route('supply.ppmp_check.approve', '') }}/' + $('#approvePpmpForm #id').val(),
                method: "POST",
                data: fd,
                cache: false,
                contentType: false,
