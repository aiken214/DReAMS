...
@can('iirup_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('supply.iirup.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.iirup.title_short') }}
            </a> 
            <a class="btn btn-primary" href="{{ route('supply.iirup.create_from_rpcppe') }}">
                {{ trans('global.add') }} {{ trans('cruds.iirup.title_short') }} {{ 'from RPCPPE' }}
            </a> 
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable" id="iirupTable">
                <thead>
                    <tr>
                        <th width="10">
...
<script>
    $(function () {
        ...
        let table = $('#iirupTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.iirup.index') }}', // Your API route
            ...
            columns: [
                ...
                { data: 'requester', name: 'requester' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('iirup_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.iirup.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('iirup_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('supply/iirup') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('iirup_delete')
                                <form action="{{ route('supply.iirup.destroy', '') }}/${data}" method="POST" 
                            ...
                            @endcan
                            @can('iirup_item_access')
                                <a class="btn btn-xs btn-success" href="{{ route('supply.iirup_item.index2', '') }}/${data}">
                                    {{ trans('global.item') }}
                                </a>
                            @endcan`;
                    },
