...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("supply.rrsp_hv.store") }}" enctype="multipart/form-data">
            @csrf
            <input type="text" name="reference" id="reference" hidden>
            <div class="form-group">
                <label class="required" for="ics_hv_id">{{ trans('cruds.ics.fields.ics_no') }}</label>
                <select class="form-control {{ $errors->has('ics_hv_id') ? 'is-invalid' : '' }}" name="ics_hv_id" id="ics_hv_id" required>
                    <option value="" disabled {{ old('ics_hv_id', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}
                    </option>
                    @foreach($icsHvs as $icsHv)
                        <option value="{{ $icsHv->id }}" 
                            data-reference="{{ $icsHv->ics_hv_no }}" 
                            data-recipient="{{ $icsHv->recipient }}" 
                            data-designation="{{ $icsHv->designation }}" 
                            data-entity_name="{{ $icsHv->entity_name }}" 
                            {{ old('ics_hv_id') == $icsHv->id ? 'selected' : '' }}>
                            {{ $icsHv->ics_hv_no }} - {{ $icsHv->recipient }} - {{ Str::limit($icsHv->ics_item_hv->description, 120) }}
                        </option>
                    @endforeach   
                </select>
                @if($errors->has('ics_hv_id'))
                    <span class="text-danger">{{ $errors->first('ics_hv_id') }}</span>
                @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const risIdDropdown = document.getElementById('ics_hv_id');
