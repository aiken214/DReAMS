...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("supply.iar_item.store") }}" enctype="multipart/form-data" id="create_form" >
            @csrf
            <input type="text" name="iar_id" id="iar_id" value="{{ $id }}" hidden> 
            <div class="form-group">
                <label class="required" for="purchase_order_item_id">{{ trans('cruds.iar_item.fields.search_item') }}</label>
                <select class="form-control {{ $errors->has('purchase_order_item_id') ? 'is-invalid' : '' }}" name="purchase_order_item_id" id="purchase_order_item_id" required>
                    <option value="" disabled {{ old('purchase_order_item_id', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}
                    </option>
                    @foreach($purchaseOrderItems as $purchaseOrderItem)
                        <option value="{{ $purchaseOrderItem->id }}" 
                            data-description="{{ $purchaseOrderItem->description }}" 
                            data-stock_no="{{ $purchaseOrderItem->stock_no }}" 
                            data-unit="{{ $purchaseOrderItem->unit }}" 
                            data-unit_price="{{ $purchaseOrderItem->unit_cost }}" 
                            data-po_quantity="{{ $purchaseOrderItem->quantity }}" 
                            {{ old('purchase_order_item_id') == $purchaseOrderItem->id ? 'selected' : '' }}>
                            {{ $purchaseOrderItem->description }} 
                        </option>
                    @endforeach   
                </select>
                @if($errors->has('purchase_order_item_id'))
                    <span class="text-danger">{{ $errors->first('purchase_order_item_id') }}</span>
                @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const purchase_order_item_idDropdown = document.getElementById('purchase_order_item_id');
        ...
        purchase_order_item_idDropdown.addEventListener('change', function () {
            const selectedOption = purchase_order_item_idDropdown.options[purchase_order_item_idDropdown.selectedIndex];
