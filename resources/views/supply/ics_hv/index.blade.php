...
@can('ics_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('supply.ics_hv.create') }}">
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable" id="icsTable">
                <thead>
                    <tr>
                        <th width="10">
...
<script>
    $(function () {
        ...
        let table = $('#icsTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.ics_hv.index') }}', // Your API route
            ...
            columns: [
                ...
                { data: 'status', name: 'status' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('ics_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.ics_hv.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('ics_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('supply/ics_hv') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('ics_delete')
                                <form action="{{ route('supply.ics_hv.destroy', '') }}/${data}" method="POST" 
                            ...
                            @endcan
                            @can('ics_item_access')
                                <a class="btn btn-xs btn-success" href="{{ route('supply.ics_item_hv.index2', '') }}/${data}">
                                    {{ trans('global.item') }}
                                </a>
                            @endcan`;
                    },
