...
@can('ics_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-secondary" href="{{ route('supply.ics_lv_print', $id) }}">
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            ...
            <input type="hidden" name="hidden_ics_lv_id" id="hidden_ics_lv_id" value="{{ $id }}"/>   
        </div>
    </div>
</div>
...
<script>
    var ics_lv_id = $('#hidden_ics_lv_id').val();   
...
<script>
    $(function () {
        ...
        let table = $('#ics_itemTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.ics_item_lv.index') }}?ics_lv_id='+ics_lv_id, // Your API route
            ...
            columns: [
                ...
                { data: 'type', name: 'type' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('ics_item_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.ics_item_lv.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('ics_item_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('supply/ics_item_lv') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan`;
                    },
