...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.iirup_item.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("supply.iirup_item.store_from_rpcppe") }}" enctype="multipart/form-data" id="create_form" >
            @csrf
            <input type="text" name="iirup_id" id="iirup_id" value="{{ $id }}" hidden>   
            <input type="text" name="particulars" id="particulars"  hidden> 
            <div class="form-group">
                <label class="required" for="rpcppe_id">{{ trans('cruds.iirup_item.fields.particulars') }}</label>
                <select class="form-control {{ $errors->has('rpcppe_id') ? 'is-invalid' : '' }}" name="rpcppe_id" id="rpcppe_id" required>
                    <option value="" disabled {{ old('rpcppe_id', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}
                    </option>
                    @foreach($rpcppes as $rpcppe)
                        <option value="{{ $rpcppe->id }}" 
                            data-date_acquired="{{ $rpcppe->par_item->par->date ?? '' }}" 
                            data-particulars="{{ $rpcppe->description }}" 
                            data-property_no="{{ $rpcppe->property_no }}" 
                            data-quantity="{{ $rpcppe->quantity_property_card }}" 
                            data-unit_cost="{{ $rpcppe->unit_value }}" 
                            {{ old('rpcppe_id') == $rpcppe->id ? 'selected' : '' }}>
                            {{ $rpcppe->par_item->par->recipient ?? '' }} - {{ $rpcppe->description }}
                        </option>
                    @endforeach   
                </select>
                @if($errors->has('rpcppe_id'))
                    <span class="text-danger">{{ $errors->first('rpcppe_id') }}</span>
                @endif
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const rpcppeIdDropdown = document.getElementById('rpcppe_id');
