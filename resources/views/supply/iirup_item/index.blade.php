...
@can('par_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('supply.iirup_item.create2', $id) }}">
                {{ trans('global.add') }} {{ trans('cruds.iirup_item.title_short') }}
            </a> 
            <a class="btn btn-primary" href="{{ route('supply.iirup_item.create_from_rpcppe', $id) }}">
                {{ trans('global.add') }} {{ trans('cruds.iirup_item.title_short') }} {{ 'from RPCPPE' }}
            </a> 
            <a class="btn btn-secondary" href="{{ route('supply.iirup_print', $id) }}">
                {{ trans('global.view') }} {{ trans('cruds.iirup.title_short') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            ...
            <input type="hidden" name="hidden_iirup_id" id="hidden_iirup_id" value="{{ $id }}"/>   
        </div>
    </div>
</div>
...
<script>
    var iirup_id = $('#hidden_iirup_id').val();   
...
<script>
    $(function () {
        ...
        let table = $('#iirup_itemTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.iirup_item.index') }}?iirup_id='+iirup_id, // Your API route
            ...
            columns: [
                ...
                { data: 'amount', name: 'amount', render: $.fn.dataTable.render.number(',', '.', 2)},
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('iirup_item_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.iirup_item.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('iirup_item_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('supply/iirup_item') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan`;
                    },
