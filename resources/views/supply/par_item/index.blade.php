...
@can('par_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-primary" href="{{ route('supply.par_item.transfer', $id) }}">
                {{ trans('global.return') }} {{ trans('global.item') }}
            </a>
            <a class="btn btn-secondary" href="{{ route('supply.par_print', $id) }}">
                {{ trans('global.view') }} {{ trans('cruds.par.title_short') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            ...
            <input type="hidden" name="hidden_par_id" id="hidden_par_id" value="{{ $id }}"/>   
        </div>
    </div>
</div>
...
<script>
    $(function () {
        ...
        let table = $('#par_itemTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.par_item.index') }}?par_id='+par_id, // Your API route
            ...
            columns: [
                ...
                { data: 'serial_no', name: 'serial_no' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('par_item_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.par_item.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('par_item_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('supply/par_item') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan`;
                    },
