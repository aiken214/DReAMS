...
@can('rrsp_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-secondary" href="{{ route('supply.rrsp_hv_print', $id) }}">
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            ...
            <input type="hidden" name="hidden_rrsp_hv_id" id="hidden_rrsp_hv_id" value="{{ $id }}"/>   
        </div>
    </div>
</div>
...
<script>
    var rrsp_hv_id = $('#hidden_rrsp_hv_id').val();   
...
<script>
    $(function () {
        ...
        let table = $('#ics_itemTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.rrsp_item_hv.index') }}?rrsp_hv_id='+rrsp_hv_id, // Your API route
            ...
            columns: [
                ...
                { data: 'type', name: 'type' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('rrsp_item_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('supply.rrsp_item_hv.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan`;
                    },
