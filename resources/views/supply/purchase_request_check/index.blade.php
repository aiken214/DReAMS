...
@can('purchase_request_check_access')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('supply.purchase_request_check.checked') }}">
                {{ trans('global.checked') }} {{ trans('cruds.purchase_request.title_singular') }} {{ trans('global.list') }}
            </a>            
        </div>
    </div>
@endcan
...
<script>
    ...
    $(function () {
        ...
        let table = $('#purchase_requestTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.purchase_request_check.index') }}',
            buttons: dtButtons,
            columns: [
                ...
                { data: 'remarks', name: 'remarks' },
                {
                    ...
                    render: function (data, type, row) {
                        return `@can('purchase_request_check_show')
                            ...
                            @endcan
                            @can('purchase_request_check_access')
                                ${(row.finalized !== null && row.checked === null && row.remarks !== 'Request for PR reversal - End User.') ? `                                    
                            ...
                            @endcan
                            @can('purchase_request_check_access')
                            ${row.remarks === 'Request for PR reversal - End User.' ? `
                                <a class="btn btn-xs btn-secondary text-white revertIcon" href="{{ route('supply.purchase_request_check.revert', '') }}/${data}">
                                    {{ trans('global.revert') }}
                                </a>
                            ` : ''}
                            @endcan`;
        ...
        $("#approvePrForm").submit(function (e) {
            ...
            $.ajax({
                url: '{{ route('supply.purchase_request_check.approve', '') }}/' + $('#approvePrForm #id').val(),
                method: "POST",
                data: fd,
                cache: false,
                contentType: false,
