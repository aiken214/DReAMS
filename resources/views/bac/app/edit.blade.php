...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("bac.app.update", [$app->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="calendar_year">{{ trans('cruds.app.fields.calendar_year') }}</label>
                <input class="form-control {{ $errors->has('calendar_year') ? 'is-invalid' : '' }}" type="text" name="calendar_year" id="calendar_year" value="{{ old('calendar_year', $app->calendar_year) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $app->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $app->type) }}" required>
                    <option value disabled {{ old('type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Ppmp::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $app->type) === (string) $key ? 'selected' : ''}}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('title'))
                    <span class="text-danger">{{ $errors->first('title') }}</span>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" type="text" name="category" id="category" value="{{ old('category', $app->category) }}" required>
                    <option value disabled {{ old('type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Ppmp::CATEGORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('category', $app->category) === (string) $key ? 'selected' : ''}}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" type="text" name="remarks" id="remarks">{{ old('remarks', $app->remarks) }}</textarea>
