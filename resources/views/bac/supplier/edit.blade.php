...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("bac.supplier.update", [$supplier->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tin') ? 'is-invalid' : '' }}" type="text" name="tin" id="tin" value="{{ old('tin', $supplier->tin) }}" required>
                @if($errors->has('tin'))
