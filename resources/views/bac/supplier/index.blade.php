...
@can('supplier_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('bac.supplier.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.supplier.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button>
            @include('csvImport.modal', ['model' => 'supplier', 'route' => 'bac.supplier.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable" id="supplierTable">
                <thead>
                    <tr>
                        <th width="10">
...
<script>
    $(function () {
        ...
        let deleteButton = {
            ...
            url: "{{ route('bac.supplier.massDestroy') }}",
        ...
        let table = $('#supplierTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.supplier.index') }}', // Your API route
            ...
            columns: [
                ...
                { data: 'tin', name: 'tin' },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('supplier_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('bac.supplier.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('supplier_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('bac/supplier') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('supplier_delete')
                                <form action="{{ route('bac.supplier.destroy', '') }}/${data}" method="POST" 
