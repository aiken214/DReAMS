...
<script>
    ...
    $(function () {
        ...
        let table = $('#ppmpTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.ppmp_final.index') }}',
            buttons: dtButtons,
            columns: [
                ...
                { data: 'remarks', name: 'remarks' },
                {
                    data: 'id',
                    render: function (data, type, row) {
                        return `@can('ppmp_final_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('bac.ppmp_final.show', '') }}/${data}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            @can('ppmp_item_final_access')
                            <a class="btn btn-xs btn-success" href="{{ route('bac.ppmp_item_final.index2', '') }}/${data}">
                                {{ trans('global.item') }}
                            </a>
                            @endcan
                            @can('ppmp_final_access')
                                ${(row.checked !== null && row.remarks === 'Approved by BAC.') ? `                                    
                            ...
                            @endcan
                            @can('ppmp_final_access')
                                ${(row.checked !== null && row.remarks !== 'Request for PPMP Reversal - End User.' && row.remarks !== 'Approved by BAC.') ? `                                    
                                    <a class="btn btn-xs btn-warning approveIcon" data-id="${data}" data-bs-toggle="modal" data-bs-target="#approvePpmpModal">
        ...
        $("#approvePpmpForm").submit(function (e) {
            ...
            $.ajax({
                url: '{{ route('bac.ppmp_final.approve', '') }}/' + $('#approvePpmpForm #id').val(),
                method: "POST",
                data: fd,
                cache: false,
                contentType: false,
