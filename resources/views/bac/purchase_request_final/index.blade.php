...
<script>
    ...
    $(function () {
        ...
        let table = $('#purchase_requestTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.purchase_request_final.index') }}',
            buttons: dtButtons,
            columns: [
                ...
                { data: 'remarks', name: 'remarks' },
                {
                    data: 'id',
                    render: function (data, type, row) {
                        return `@can('purchase_request_final_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('bac.purchase_request_final.show', '') }}/${data}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            @can('purchase_request_item_final_access')
                            <a class="btn btn-xs btn-success" href="{{ route('bac.purchase_request_item_final.index2', '') }}/${data}">
                                {{ trans('global.item') }}
                            </a>
                            @endcan
                            @can('purchase_request_final_access')
                                ${(row.approved !== null && row.added === null && row.remarks !== 'Request for PR Reversal - End User.') ? `                                     
        ...
        $("#approvePrForm").submit(function (e) {
            ...
            $.ajax({
                url: '{{ route('bac.purchase_request_final.approve', '') }}/' + $('#approvePrForm #id').val(),
                method: "POST",
                data: fd,
                cache: false,
                contentType: false,
