...
@can('request_for_quotation_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('bac.request_for_quotation.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.request_for_quotation.title_singular') }}
            </a>         
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.request_for_quotation.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable" id="request_for_quotationTable">
                <thead>
                    <tr>
                        <th width="10">
...
<script>
    $(function () {
        ...
        let deleteButton = {
            ...
            url: "{{ route('bac.request_for_quotation.massDestroy') }}",
        ...
        let table = $('#request_for_quotationTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.request_for_quotation.index') }}', // Your API route
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('request_for_quotation_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('bac.request_for_quotation.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('request_for_quotation_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('bac/request_for_quotation') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('request_for_quotation_delete')
                                <form action="{{ route('bac.request_for_quotation.destroy', '') }}/${data}" method="POST" 
                            ...
                            @endcan
                            @can('request_for_quotation_item_access')
                                <a class="btn btn-xs btn-success" href="{{ route('bac.request_for_quotation_item.index2', '') }}/${data}">
                                    {{ trans('global.item') }}
                                </a>
                            @endcan`;
                    },
