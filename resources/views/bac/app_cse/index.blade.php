    ...
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            @if(empty($finalized) || !$finalized)
                <a class="btn btn-success" href="{{ route('bac.app_cse.create2', $app_id) }}">
                    {{ trans('global.add') }} {{ trans('cruds.ppmp.title_singular') }} {{ '- CSE'}}
                </a>
            @endif
                <a class="btn btn-secondary" href="{{ route('bac.app_cse_print', $app_id) }}">
...
<script>
    ...
    $(function () {
        ...
        let table = $('#ppmpTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.app_cse.index') }}?app_id='+app_id, // Your API route
            buttons: dtButtons,
            columns: [
                ...
                { data: 'budget_alloc', name: 'budget_alloc', render: $.fn.dataTable.render.number(',', '.', 2)},
                {
                    ...
                    render: function (data, type, row) {
                        return `@can('app_cse_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('bac.app_cse.show', '') }}/${data}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            @can('app_cse_delete')
                                ${row.finalized === null ? `
                                    <form action="{{ route('bac.app_cse.destroy', '') }}/${data}" method="POST" 
