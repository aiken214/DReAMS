...
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-secondary" href="{{ route('bac.request_for_quotation_print', $id) }}">
            {{ trans('global.view') }} {{ trans('cruds.request_for_quotation.title_singular') }}
        </a>
    </div>
</div>
<div class="card">    
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable" id="purchase_request_itemTable">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('cruds.purchase_request_item.fields.unit_price') }} {{ 'ABC' }}
                        </th>
                        <th>
            ...
            <input type="hidden" name="hidden_purchase_request_id" id="hidden_purchase_request_id" value="{{ $purchase_request_id }}"/>   
        </div>
    </div>
</div>
...
<script>
    $(function () {
        ...
        let table = $('#purchase_request_itemTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.request_for_quotation_item.index') }}?purchase_request_id='+purchase_request_id, // Your API route
