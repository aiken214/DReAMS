...
@can('purchase_request_verify_access')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('budget.purchase_request_verify.verified') }}">
                {{ trans('global.verified') }} {{ trans('cruds.purchase_request.title_singular') }} {{ trans('global.list') }}
            </a>            
        </div>
    </div>
@endcan
...
<script>
    ...
    $(function () {
        ...
        let table = $('#purchase_requestTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.purchase_request_verify.index') }}',
            buttons: dtButtons,
            columns: [
                ...
                { data: 'remarks', name: 'remarks' },
                {
                    data: 'id',
                    render: function (data, type, row) {
                        return `@can('purchase_request_verify_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('budget.purchase_request_verify.show', '') }}/${data}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            @can('purchase_request_item_verify_access')
                            <a class="btn btn-xs btn-success" href="{{ route('budget.purchase_request_item_verify.index2', '') }}/${data}">
                                {{ trans('global.item') }}
                            </a>
                            @endcan
                            @can('purchase_request_verify_access')
                                ${(row.checked !== null && row.verified === null && row.remarks !== 'Request for PR reversal - End User.') ? `                                     
        ...
        $("#approvePrForm").submit(function (e) {
            ...
            $.ajax({
                url: '{{ route('budget.purchase_request_verify.approve', '') }}/' + $('#approvePrForm #id').val(),
                method: "POST",
                data: fd,
                cache: false,
                contentType: false,
