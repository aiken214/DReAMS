...
@section('content')
@can('purchase_request_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            @if(empty($data->finalized))
            <a class="btn btn-success" href="{{ route('user.purchase_request_item.create2', $id) }}">
                {{ trans('global.add') }} {{ trans('cruds.purchase_request_item.title_singular') }}
            </a>
            @endif
...
<script>
    $(function () {
        ...
        @can('purchase_request_item_delete')
        let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
        let deleteButton = {
            text: deleteButtonTrans,
            url: "{{ route('user.purchase_request_item.massDestroy') }}",
        ...
        let table = $('#purchase_request_itemTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.purchase_request_item.index') }}?purchase_request_id='+purchase_request_id, // Your API route
            ...
            columns: [
                ...
                { data: 'total_cost', name: 'total_cost', render: $.fn.dataTable.render.number(',', '.', 2)},
                {
                    ...
                    if (row.purchase_request.finalized === null) {
                        return `
                            @can('purchase_request_item_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('user.purchase_request_item.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('purchase_request_item_edit')
                                <a class="btn btn-xs btn-info" href="{{ url('user/purchase_request_item') }}/${data}/edit">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('purchase_request_item_delete')
                                <form action="{{ route('user.purchase_request_item.destroy', '') }}/${data}" method="POST" 
