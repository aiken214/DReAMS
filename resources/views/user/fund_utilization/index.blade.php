...
<script>
    $(function () {
        ...
        let table = $('#fundTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.fund_utilization.index') }}?user_id='+user_id, // Your API route
            ...
            columns: [
                ...
                { data: 'legal_basis', name: 'legal_basis' },  
                { data: 'ppa', name: 'ppa' },
                ...
                }},
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            @can('fund_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('user.fund_utilization.show', '') }}/${data}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('fund_access')
                                <a class="btn btn-xs btn-success" href="{{ route('user.fund_utilization_details.index2', '') }}/${data}">
