...
@can('purchase_request_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <form action="{{ route('supply.purchase_request_migration.execute') }}" method="POST">
                @csrf
                <button type="submit" class="btn btn-success">
                    {{ 'Migrate' }} {{ trans('cruds.purchase_request.title_singular') }}
                </button>
            </form>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        let table = $('#purchase_requestTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.purchase_request_migration.index') }}', // Your API route
