...
@can('purchase_order_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <form action="{{ route('supply.purchase_order_migration.execute') }}" method="POST">
                @csrf
                <button type="submit" class="btn btn-success">
                    {{ 'Migrate' }} {{ trans('cruds.purchase_order.title_singular') }}
                </button>
            </form>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        let table = $('#purchase_orderTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('api.purchase_order_migration.index') }}', // Your API route
